...
import { supabaseDataService, USE_SUPABASE } from '../../services/dataService';
...
export default function StockView({
  ...
  const [activeLotBalances, setActiveLotBalances] = useState<ActiveLotBalance[]>([]);

  useEffect(() => {
    if (activeTab === 'status' && USE_SUPABASE) {
      supabaseDataService.getActiveRawMaterialLotBalances()
        .then(data => setActiveLotBalances(data))
        .catch(err => console.error("Active lot balances fetch error:", err));
    }
  }, [activeTab, rawMaterialReceipts, rawMaterialLots]);

  const activeLotsMap = useMemo(() => {
    const map: Record<string, ActiveLotBalance[]> = {};
    const items: ActiveLotBalance[] = USE_SUPABASE ? activeLotBalances : (rawMaterialLots || [])
      .filter(l => !l.isDeleted && l.quantityRemaining > 0)
      .map(l => {
        const receipt = (rawMaterialReceipts || []).find(r => r.id === l.rawMaterialReceiptId);
        const supplier = receipt ? (suppliers || []).find(s => s.id === receipt.supplierId) : null;
        const rm = rawMaterials.find(r => r.id === l.rawMaterialId);
        return {
          id: l.id,
          internalLotNo: l.internalLotNo,
          quantityReceived: l.quantityReceived,
          quantityRemaining: l.quantityRemaining,
          unit: l.unit || rm?.unit || 'kg',
          unitPrice: l.unitPrice,
          kunyeStatus: l.kunyeStatus,
          kunyeNumber: l.kunyeNumber || null,
          rawMaterialId: l.rawMaterialId,
          rawMaterialName: rm?.name || '—',
          rawMaterialReceiptId: l.rawMaterialReceiptId,
          receiptDate: receipt?.receiptDate || l.createdAt,
          invoiceNumber: receipt?.invoiceNumber || null,
          dispatchNoteNumber: receipt?.dispatchNoteNumber || null,
    ...
    items.forEach(lot => {
      if (!map[lot.rawMaterialId]) {
        map[lot.rawMaterialId] = [];
      }
      map[lot.rawMaterialId].push(lot);
    });

    Object.keys(map).forEach(rmId => {
      map[rmId].sort((a, b) => {
    ...
    return map;
  }, [activeLotBalances, rawMaterialLots, rawMaterialReceipts, suppliers, rawMaterials]);
                ...
                <tbody className="divide-y divide-slate-100 text-slate-600">
                  {filteredMaterials.length === 0 ? (
                    ...
                    filteredMaterials.map((rm) => {
                      const stock = currentStocks[rm.id] || 0;
                      const activeLotsForRm = activeLotsMap[rm.id] || [];
                        ...
                        <React.Fragment key={rm.id}>
                          <tr className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="py-3.5 px-4 text-center">
                              <button
                                type="button"
                                disabled={activeLotsForRm.length === 0}
                                onClick={() => activeLotsForRm.length > 0 && toggleExpandMaterialLots(rm.id)}
                                className={`inline-flex items-center gap-1 font-bold text-[11px] px-2 py-0.5 rounded-md transition-colors ${
                          ...
                          {isExpanded && activeLotsForRm.length > 0 && (
                                    ...
                                    <table className="w-full text-left text-[11px] border-collapse">
                                      ...
                                      <tbody className="divide-y divide-slate-100">
                                        {activeLotsForRm.map(lot => {
                                          ...
                                          const docNo = lot.invoiceNumber || lot.dispatchNoteNumber || '—';
                                          const rDate = lot.receiptDate ? formatDate(lot.receiptDate) : '—';
